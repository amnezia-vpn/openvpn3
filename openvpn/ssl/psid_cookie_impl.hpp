...
namespace openvpn {
  ...
  public:
      ...
      if (pcfg_.key_direction >= 0)
        {
          ...
          const unsigned int key_dir =
            pcfg_.key_direction ? OpenVPNStaticKey::INVERSE : OpenVPNStaticKey::NORMAL;
    ...
    virtual Intercept intercept(ConstBuffer& pkt_buf, const PsidCookieAddrInfoBase& pcaib) override
    {
      ...
      if (not_tls_auth_mode_) {  // test discovered in TLSAuthPreValidate
  ...
  private:
    ...
    Intercept process_clients_initial_reset(ConstBuffer& pkt_buf, const PsidCookieAddrInfoBase& pcaib)
    {
      ...
      bool pkt_hmac_valid =
        ta_hmac_recv_->ovpn_hmac_cmp(pkt_buf.c_data(), pkt_buf.size(),
                                     1 + SID_SIZE, hmac_size, long_pktid_size_);
      if (!pkt_hmac_valid) {
        // JMD_TODO: log failure?  Logging DDoS?
      ...
      static const size_t reqd_packet_size
        // [op_field] [cli_psid]      [HMAC]     [cli_auth_pid]           [cli_pid]
      ...
      ConstBuffer recv_buf_copy(pkt_buf.data() + 1, pkt_buf.size() - 1, true);
      // decapsulate_tls_auth
      const ProtoSessionID cli_psid(recv_buf_copy);
      recv_buf_copy.advance(hmac_size);
      PacketID cli_auth_pid;  // a.k.a, replay_packet_id in draft RFC
      cli_auth_pid.read(recv_buf_copy, PacketID::LONG_FORM);
      PacketID cli_pid;  // a.k.a., packet_id in draft RFC
      cli_pid.read(recv_buf_copy, PacketID::SHORT_FORM);
      ...
      const id_t cli_net_id = htonl(cli_pid.id);
      ...
      // write hmac
      ta_hmac_send_->ovpn_hmac_gen(send_buf.data(), send_buf.size(), 1 + SID_SIZE,
                                   ta_hmac_send_->output_size(), long_pktid_size_);
    ...
    Intercept process_clients_server_reset_ack(ConstBuffer& pkt_buf, const PsidCookieAddrInfoBase& pcaib)
    {
      ...
      static const size_t reqd_packet_size
        // [op_field] [cli_psid]      [HMAC]     [cli_auth_pid] [acked] [srv_psid]           [cli_pid]
    ...
    ProtoSessionID calculate_session_id_hmac(const ProtoSessionID& cli_psid,
                                             const PsidCookieAddrInfoBase& pcaib, int offset)
    {
      hmac_ctx_.reset();
                               ...
                               const PsidCookieAddrInfoBase& pcaib)
    {
      ...
      for (int offset = 0; offset <= 1; ++offset)
        {
          ProtoSessionID calc_psid =
            calculate_session_id_hmac(cli_psid, pcaib, offset);
